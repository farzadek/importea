...
app.controller('mainCtrl', function($scope, $cookieStore, $http, $timeout, grPortalService) {
    ...
    $scope.changeLanguage = function() {
        if ($scope.lang) { $scope.lang = 0; } else { $scope.lang = 1; }
    }
                            ...
                            if (response.data.length) {
                                ...
                                grPortalService.getUsers('{"active":true}').then(
                                    function(response) {
                                        ...
                                        $.ajax({
                                            url: 'https://api.mlab.com/api/1/databases/greenportal/collections/users?apiKey=' + $scope.dbApiKey,
    ...
    $scope.openUserBox = function() {
        if ($scope.currentUser) {
            ...
            grPortalService.getFacteurs(q).then(
                function(res) {
                    if (res.status != 200) { alert($scope.titles.alerts.informationLoad[$scope.lang]); } else {
    ...
    $scope.sorting_command = function(colSelected) {
        switch (colSelected) {
            case 0:
                $scope.sortType_command = 'date';
                break;
            case 1:
                $scope.sortType_command = 'cn';
                break;
            case 2:
                $scope.sortType_command = 'payed';
                break;
            case 3:
                $scope.sortType_command = 'sent';
                break;
        }
    ...
    $scope.sorting_user = function(colSelected) {
        switch (colSelected) {
            case 0:
                $scope.sortType_user = 'id';
                break;
            case 1:
                $scope.sortType_user = 'lastname';
                break;
            case 2:
                $scope.sortType_user = 'special';
                break;
            case 3:
                $scope.sortType_user = 'active';
                break;
        }
    ...
    $scope.sorting_message = function(colSelected) {
        switch (colSelected) {
            case 0:
                $scope.sortType_message = 'date';
                break;
            case 1:
                $scope.sortType_message = 'name';
                break;
            case 2:
                $scope.sortType_message = 'phone';
                break;
            case 3:
                $scope.sortType_message = 'email';
                break;
        }
